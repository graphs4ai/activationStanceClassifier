...
def load_artifact_data(artifact_dir: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    baseline_pairs = _pick_single_file(artifact_dir, "baseline_pair_results_*.csv")
    ...
    if "valid" in baseline_valid.columns:
        baseline_valid = baseline_valid[baseline_valid["valid"].astype(str) == "True"]
    ...
    intervention_pis = intervention_valid["polarization_index"].to_numpy(dtype=float)
...
def compute_comparison_stats(
    ...
    if n_baseline == n_other and n_baseline > 0:
        try:
            test_stat, p_value = stats.wilcoxon(baseline_pis, other_pis)
            test_type = "Wilcoxon"
        except ValueError:
    ...
    return float(test_stat), float(p_value), test_type, float(effect_size)
...
def create_composite_plot(
    ...
    bp = ax.boxplot(
        data_to_plot,
        labels=["Baseline", "Maximization", "Minimization"],
        patch_artist=True,
        widths=0.6,
    ...
    ax.set_xticks([1, 2, 3])
    ax.set_xticklabels(["Base", "Max", "Min"], fontsize=14, fontweight="bold")
...
def main() -> None:
    ...
    baseline_mean_delta = float(np.mean(max_baseline_pis) - np.mean(min_baseline_pis))
    ...
    create_composite_plot(baseline_pis, maximization_pis, minimization_pis, plot_path)
    ...
    print(f"Base-Min ({type_min}): stat={stat_min:.4f}, p={p_min:.6f}, d={effect_min:.3f}")
    print(f"Base-Max ({type_max}): stat={stat_max:.4f}, p={p_max:.6f}, d={effect_max:.3f}")
